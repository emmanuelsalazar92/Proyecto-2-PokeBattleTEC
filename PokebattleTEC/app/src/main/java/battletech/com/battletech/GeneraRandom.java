...
/**
 * Created by Emmanuel on 17/11/2014.
 */
public class GeneraRandom extends AsyncTask<Integer, Void, Boolean> {
    private Activity activity;
    private BattledexListAdapter adapter;
    private List<Battledex> movies = new ArrayList<Battledex>();
    private String URL = "http://poke-battle.herokuapp.com/api/pokemon/map/";
    ...
    int idUsuario;
    public GeneraRandom(BattleTechActivityMain activity, BattledexListAdapter adapter, int id){
        Log.d("-------------------", "LoadBattle");
    ...
    protected Boolean doInBackground(Integer... parameters) {
        ...
        try {
            ...
            if(statusLine.getStatusCode() == HttpStatus.SC_OK){
                ...
                JSONArray jArray = jsonObj.getJSONArray("pokemons");
                for(int i=0; i < jArray.length(); i++) {
                    JSONObject jObject = jArray.getJSONObject(i);
                    String name = jObject.getString("pokemon");
                    //String image = jObject.getString("CoverImage");
                    Battledex lista = new Battledex();
                    lista.set_SPokemon(_PokemonName[Integer.parseInt(name)-1]);
                    // p.setImage(image);
                    this.movies.add(lista);
                }
                Log.d("-------------------","true");
                return true;
            }else{
